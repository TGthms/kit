...
- Theme persists in localStorage (`theme` = applied value, `kit-theme-context` = user intent). System is the default. A manual Dark choice always sticks. A manual Light choice is honored only while the OS is light and local time is 05:00–21:59; if the OS is dark or it is night (22:00–04:59), Light is forced to Dark and restored when that condition lifts. Light/dark switches use a circular View Transition wipe from the control (instant when View Transitions are missing, reduced motion is on, or the force keeps the current appearance).
...
- Mobile PWA tab bar is a floating glass capsule above the home indicator (content scrolls underneath). The active tab and 2-item segments (timer mode, appearance) use a gliding pill; switches use a short overshoot on the thumb.
