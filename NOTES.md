...
- Mobile PWA tab bar is a floating glass capsule just above the home indicator (content scrolls underneath). The capsule uses a thinner fill and a stronger blur than other chrome so the material reads as iOS ultra-thin glass. Switching tabs springs the gliding pill (slight overshoot) and pops the selected icon; reduced motion skips both. Re-tapping the current tab scrolls to the top. The bar slides away while the virtual keyboard covers the bottom. Compact tool headers on mobile use the same glass as the top chrome. 2-item segments (timer mode, appearance) still use a gliding pill; switches use a short overshoot on the thumb. The document scroller keeps native rubber-band bounce at the top and bottom; overscroll is not locked in the installed PWA.
