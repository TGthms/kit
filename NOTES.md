...
- Mobile PWA tab bar is a floating glass capsule just above the home indicator (content scrolls underneath), lifted 2px off the indicator. The capsule uses a thinner fill and a stronger blur than other chrome so the material reads as iOS ultra-thin glass. The selected-tab highlight is a concentric `rounded-full` stadium that matches the bar’s circular ends. Switching tabs springs the gliding pill (slight overshoot) and pops the selected icon; reduced motion skips both. Re-tapping the current tab scrolls to the top. The bar slides away while the virtual keyboard covers the bottom. Compact tool headers on mobile are a fixed glass overlay (not sticky, not in document flow); only the back control takes taps, so scrolling stays smooth. 2-item segments (timer mode, appearance) still use a gliding pill; switches use a short overshoot on the thumb. The document scroller keeps native rubber-band bounce at the top and bottom; overscroll is not locked in the installed PWA.
